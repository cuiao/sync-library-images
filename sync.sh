...
REGISTRY_LIBRARY="${REGISTRY_DOMAIN}/microsoft"
REPO_PATH=$2
: ${REPO_PATH:=${PWD}}
...
diff_images() {
    ...
    IMAGES=$(grep -Er "^Tags:|^SharedTags:" library | sed 's|library/||g;s|:Tags||g;s|:SharedTags||g;s| ||g')
...
skopeo_copy() {
    if skopeo copy --all --insecure-policy --src-tls-verify=false --dest-tls-verify=false -q docker://$1 docker://$2; then
...
sync_images() {
    ...
    for image in ${IMAGES}; do
        ...
        if skopeo_copy mcr.microsoft.com/${name}:${tags} ${REGISTRY_LIBRARY}/${name}:${tags}; then
